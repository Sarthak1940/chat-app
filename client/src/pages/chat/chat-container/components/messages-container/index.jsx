...
import { BACKEND_URL, GET_CHANNEL_MESSAGES, GET_MESSAGES_ROUTE } from "@/utils/constants";
...
const MessageContainer = () => {
  ...
  const downloadFile = async (url) => {
    ...
    const response = await apiClient.get(`${BACKEND_URL}/${url}`, {
      responseType: "blob",
  ...
  const renderDMMessages = (message) => (
    <div className={`${message.sender !== selectedChatData._id ? "text-right": "text-left"}`}>
      ...
      {message.messageType === "file" && (<div className={`${message.sender !== selectedChatData._id ? 
          ...
          border inline-block p-4 rounded my-1 max-w-[50%] break-words`}>
            ...
            onClick={() => {
              ...
              <img src={`${BACKEND_URL}/${message.fileUrl}`} height={300} width={300}/>
            </div> : 
      ...
      <div className="text-xs text-frey-600">
          ...
          <div className="flex items-center gap-3">
            <Avatar className="h-8 w-8 rounded-full overflow-hidden">
              {message.sender.image && ( 
                ...
                <AvatarFallback className={`uppercase h-8 w-8 text-lg flex items-center justify-center ${getcolor(message.sender.color)}`} style={{borderRadius: "50%"}}>
  ...
  return <div className="flex-1 overflow-y-auto scrollbar-hidden p-4 px-8 md:w-[65vw] lg:w-[70vw] xl:[80vw] w-full">
    {renderMessages()}
    <div ref={scrollRef}/>
