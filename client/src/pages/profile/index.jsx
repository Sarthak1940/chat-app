...
import { BACKEND_URL, REMOVE_PROFILE_IMAGE_ROUTE, UPDATE_IMAGE_ROUTE, UPDATE_PROFILE_ROUTE } from "@/utils/constants";
...
const Profile = () => {
  ...
  const handleImageChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        const response = await apiClient.post(UPDATE_IMAGE_ROUTE, formData, {withCredentials: true});
  ...
  return <div className="bg-[#1b1c24] h-[100vh] flex items-center justify-center flex-col gap-10">
    <div className="flex flex-col gap-10 w-[80vw] md:w-max">
      ...
      <div className="grid grid-cols-2">
        <div className="h-full w-32 md:w-48 md:h-48 flex justify-center items-center relative"
        ...
        onMouseLeave={() => setHovered(false)}>
          <Avatar className="h-32 w-32 md:h-48 md:w-48 rounded-full overflow-hidden">
            {image ? 
              ...
              /> : 
              <div className={`uppercase h-32 w-32 md:w-48 md:h-48 text-5xl border-[1px] flex items-center justify-center ${getcolor(selectedColor)}`} style={{borderRadius: "50%"}}>
